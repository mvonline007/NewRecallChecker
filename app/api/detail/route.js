...
export async function GET(req) {
  ...
  try {
    const { distributeursRaw, distributeursList } = await fetchDistributeurInfo(url);
    return Response.json(
      { distributeursRaw, distributeursList, versions: { api: VERSION, distributeur: DISTRIBUTEUR_VERSION } },
      { status: 200 }
    );
  } catch (error) {
