...
export function normalizeRecipientConfigs(raw) {
  ...
  if (typeof raw === "string") {
    return raw
      ...
      .filter(Boolean)
      .map((email) => ({ email, distributeurs: [], onlyNewItems: false }));
  }
  ...
  return raw
    .map((entry) => {
      ...
      if (typeof entry === "string") {
        return { email: entry.trim(), distributeurs: [], onlyNewItems: false };
      }
      const email = typeof entry.email === "string" ? entry.email.trim() : "";
      if (!email) return null;
      const distributeurs = normalizeDistributeurList(entry.distributeurs);
      const onlyNewItems = Boolean(entry.onlyNewItems);
      return { email, distributeurs, onlyNewItems };
    })
    .filter((entry) => {
