...
function buildItemCard(item) {
  ...
  return `
    <table role="presentation" width="100%" style="border:1px solid #e2e8f0;border-radius:12px;margin:0;background:#ffffff;">
      <tr>
...
function buildItemGrid(items) {
  ...
  for (let i = 0; i < items.length; i += 2) {
    const first = buildItemCard(items[i]);
    const second = items[i + 1] ? buildItemCard(items[i + 1]) : "";
    ...
    rows.push(`
      <table role="presentation" width="100%" style="border-collapse:collapse;">
        <tr>
          <td width="50%" style="vertical-align:top;padding:0 8px 16px 0;">${first}</td>
          <td width="50%" style="vertical-align:top;padding:0 0 16px 8px;">${secondCell}</td>
        </tr>
      </table>
    `);
  }
...
function buildSection(section) {
  ...
  const itemMarkup = items.length ? buildItemGrid(items) : "";
