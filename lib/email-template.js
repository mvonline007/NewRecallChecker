...
function buildItemCard(item) {
  ...
  const description = truncateText(stripHtml(item?.descriptionHtml || ""), DEFAULT_DESCRIPTION_LIMIT);
  ...
  const descriptionBlock = descriptionHtml
    ? `<div style="font-size:14px;line-height:1.5;color:#334155;margin:0 0 12px;">${descriptionHtml}</div>`
    : "";

  const motifBlock = motifHtml
    ? `<div style="font-size:13px;line-height:1.5;color:#475569;margin:0 0 12px;"><strong>Motif du rappel:</strong> ${motifHtml}</div>`
    : "";
